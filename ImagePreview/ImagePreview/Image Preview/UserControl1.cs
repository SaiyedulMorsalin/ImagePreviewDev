...
namespace Image_Preview
{
    public partial class UserControl1 : UserControl
    {
        ...
        {
            const int bufferSize = 128 * 1024;  // Buffer size for file copy
        ...
        public async Task DirectoryLoad(string path)
        {
            ...
            foreach (var file in files)
            {
                if (extsn.Any(ext => file.Extension.ToLower().Contains(ext.ToLower())))
                {
                    ...
                    string destinationFile = System.IO.Path.Combine(Filepath, file.Name);
        ...
        public async Task DirectoryLoadArray(string[] paths)
        {

            MessageBox.Show($"Checking array: '{paths}'", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        public async void LoadDirectoriesArr()
        {
            string[] testPaths = { "C:\\Users\\mdsai\\OneDrive\\Desktop\\dll_prop.jpg", @"C:\Users\mdsai\OneDrive\Desktop\errors.jpg" };
            await DirectoryLoadArray(testPaths);
        }
