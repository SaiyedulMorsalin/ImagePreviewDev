        ...
        private async Task DirectoryLoad(string path)
        {
            ...
            foreach (var file in files)
            {
                if (extsn.Any(ext => file.Extension.ToLower().Contains(ext.ToLower())))
                {
                    ...
                    Button thumb_btn = new Button();


                    Image thumbnail = await GetThumbnailAsync(file.FullName);
                    thumb_btn.BackgroundImage = thumbnail;
                    thumb_btn.BackgroundImageLayout = System.Windows.Forms.ImageLayout.Zoom;
                    thumb_btn.FlatStyle = System.Windows.Forms.FlatStyle.Flat;
                    thumb_btn.Size = new System.Drawing.Size(100, 100);
                    thumb_btn.UseVisualStyleBackColor = true;
                    flowLayoutPanel1.Controls.Add(thumb_btn);
